...
```shell
docker build --target dev -t bizcloud-frontend:dev .
docker build --target test -t bizcloud-frontend:test .
docker build --target prod -t bizcloud-frontend:<VERSION> .
```
...
docker run \
  ...
  bizcloud-frontend:dev
```

To run the test image:

```shell
docker run --rm -it --init bizcloud-frontend:test
```

To run the prod image:

```shell
docker run --rm -p 8080:8080 -it --init bizcloud-frontend:<VERSION>
```
